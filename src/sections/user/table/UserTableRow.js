...
export default function UserTableRow({ row, refreshTable, handleUpdate, index, page, rowsPerPage }) {
    const { getFetcher } = useSwrFetcher();
    const { enqueueSnackbar } = useSnackbar();
    const [confirmModal, setConfirmModal] = useState(false);
    const [openMenu, setOpenMenuActions] = useState(null);
    const { id, lastname, firstname, organizationName, position, email, phoneNumber, userRole, username } = row;
